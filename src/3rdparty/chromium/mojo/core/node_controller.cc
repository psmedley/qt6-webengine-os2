...
#if !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA) && !BUILDFLAG(IS_OS2)
...
void NodeController::SendBrokerClientInvitationOnIOThread(
  ...
  scoped_refptr<NodeChannel> channel = NodeChannel::Create(
      ...
      process_error_callback);
#else   // !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA) && !BUILDFLAG(IS_OS2)
  scoped_refptr<NodeChannel> channel = NodeChannel::Create(
      this, std::move(connection_params), Channel::HandlePolicy::kAcceptHandles,
      io_task_runner_, process_error_callback);
#endif  // !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_NACL) && !BUILDFLAG(IS_FUCHSIA) && !BUILDFLAG(IS_OS2)
